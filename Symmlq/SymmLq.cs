using System;
using static ChacoSharp.Symmlq.SymMlqHelper;

namespace ChacoSharp.Symmlq
{
    public static unsafe class SymmLq
    {
        private const int C1 = 1;
        private const double Cb4 = 1.0;
        private const double Cb18 = 1.0 / 3.0;
        private static double alfa, diag, dbar, beta, gbar, oldb, epsa;
        private static double gmin, gmax, zbar, epsr, epsx, beta1;
        private static double gamma, s, t, delta, z, denom;
        private static double bstep;
        private static double epsln;
        private static double tnorm, cs, ynorm2, sn, cgnorm;
        private static double snprod, lqnorm, qrnorm, eps, rhs1, rhs2;
        ...
        /// <param name="goodb"></param>
        /// <param name="precon"></param>
        /// <param name="shift"></param>
        /// <param name="nout"></param>
        /// <param name="itnlim"></param>
        ...
        /// <param name="a"></param>
        /// <param name="vwsqrt"></param>
        /// <param name="orthlist"></param>
        /// <param name="macheps"></param>
        /// <param name="normxlim"></param>
        /// <param name="itnmin"></param>
        public static void symmlq(int n, double* b, double* r1, double* r2, double* v, double* w, double* x,
            double* y, double* work, bool checka, bool goodb, bool precon,
            double* shift, int* nout, int* itnlim, double* rtol, int* istop,
            int* itn, double* anorm, double* acond, double* rnorm, double* ynorm, double* a,
            double* vwsqrt, double* orthlist, double* macheps, double* normxlim, int* itnmin)
        {
            ...
            /*                        This is Math.Sqrt( x'Mx ).  If precon is false, */
            ...
            *ynorm = 0.0;
            ...
            chdcopy(n, &b[1], C1, &y[1], C1);
            chdcopy(n, &b[1], C1, &r1[1], C1);
            if (precon)
            {
                msolve(n, &r1[1], &y[1]);
            }
            ...
            /*        b1  = zero */
            /*     end if */
            beta1 = ch_ddot(n, &r1[1], C1, &y[1], C1);
            /*     See if msolve is symmetric. */
            if (checka && precon)
            {
                msolve(n, &y[1], &r2[1]);
                s = ch_ddot(n, &y[1], C1, &y[1], C1);
                t = ch_ddot(n, &r1[1], C1, &r2[1], C1);
                d__1 = s - t;
                z = (Math.Abs(d__1));
                epsa = (s + eps) * Math.Pow(eps, Cb18);
                if (z > epsa)
                {
                    *istop = 7;
                    goto L900;
            ...
            d__1 = -(*shift);
            chdaxpy(n, d__1, &v[1], C1, &y[1], C1);
            alfa = ch_ddot(n, &v[1], C1, &y[1], C1);
            d__1 = -alfa / beta1;
            chdaxpy(n, d__1, &r1[1], C1, &y[1], C1);
            /*     Make sure  r2  will be orthogonal to the first  v. */
            z = ch_ddot(n, &v[1], C1, &y[1], C1);
            s = ch_ddot(n, &v[1], C1, &v[1], C1);
            d__1 = -z / s;
            chdaxpy(n, d__1, &v[1], C1, &y[1], C1);
            chdcopy(n, &y[1], C1, &r2[1], C1);
            if (precon)
            {
                msolve(n, &r2[1], &y[1]);
            }
            ...
            denom = Math.Sqrt(s) * chdnrm2(n, &r2[1], C1) + eps;
            s = z / denom;
            t = ch_ddot(n, &v[1], C1, &r2[1], C1) / denom;
...
/*     Estimate various norms and test for convergence. */
            ...
            *ynorm = Math.Sqrt(ynorm2);
            ...
            /* Computing 2nd power */
            d__2 = rhs2;
            lqnorm = Math.Sqrt(d__1 * d__1 + d__2 * d__2);
            qrnorm = snprod * beta1;
            ...
            if (*istop == 0)
            {
                ...
                if (*istop != 4 && *ynorm >= *normxlim && *normxlim != 0.0)
                {
                    *istop = 9;
                }
            }
            ...
            if (cgnorm <= epsx * 10.0f)
            {
                goto L400;
            }

            if (cgnorm <= epsr * 10.0f)
            {
                goto L400;
            }
...
/*     and set up  y  for the next iteration. */
            ...
            /* Computing 2nd power */
            d__2 = beta;
            tnorm = tnorm + d__1 * d__1 + d__2 * d__2 * 2.0;
            d__1 = -alfa / beta;
            chdaxpy(n, d__1, &r2[1], C1, &y[1], C1);
            chdcopy(n, &r2[1], C1, &r1[1], C1);
            chdcopy(n, &y[1], C1, &r2[1], C1);
            if (precon)
            {
                msolve(n, &r2[1], &y[1]);
            ...
            /* Computing 2nd power */
            d__2 = oldb;
            gamma = Math.Sqrt(d__1 * d__1 + d__2 * d__2);
            cs = gbar / gamma;
            sn = oldb / gamma;
...
/*     EXCEPT if the iteration limit interferes. */
            ...
            if (cgnorm <= lqnorm)
            {
                ...
                /* Computing 2nd power */
                d__1 = zbar;
                *ynorm = Math.Sqrt(ynorm2 + d__1 * d__1);
                *rnorm = cgnorm;
                chdaxpy(n, zbar, &w[1], C1, &x[1], C1);
            }
            else
            {
                *rnorm = lqnorm;
