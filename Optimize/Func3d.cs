...
{
    public static unsafe class Func3d
    {
        private static double stheta, ctheta; /* sign and cosine of angle */
        private static double sphi, cphi; /* sign and cosine of angle */
        private static double sgamma, cgamma; /* squares of sign and cosine of angle */
        private static double onemcg; /* intermediate value */

        private static double a1, a2, a3; /* linear mapping coefficients */
        private static double b1, b2, b3; /* linear mapping coefficients */
        private static double c1, c2, c3; /* linear mapping coefficients */

        private static double a1t, a2t, a3t; /* theta first derivatives */
        private static double b1t, b2t, b3t; /* theta first derivatives */
        private static double c1t, c2t, c3t; /* theta first derivatives */

        private static double a1p, a2p, a3p; /* phi first derivatives */
        private static double b1p, b2p, b3p; /* phi first derivatives */
        private static double c1p, c2p, c3p; /* phi first derivatives */

        private static double a1g, a2g, a3g; /* gamma first derivatives */
        private static double b1g, b2g, b3g; /* gamma first derivatives */
        private static double c1g, c2g, c3g; /* gamma first derivatives */

        private static double a1tt, a2tt, a3tt; /* theta/theta second derivatives */
        private static double b1tt, b2tt, b3tt; /* theta/theta second derivatives */
        private static double c1tt, c2tt, c3tt; /* theta/theta second derivatives */
        private static double a1pp, a2pp, a3pp; /* phi/phi second derivatives */
        private static double b1pp, b2pp, b3pp; /* phi/phi second derivatives */
        private static double c1pp, c2pp, c3pp; /* phi/phi second derivatives */
        private static double a1gg, a2gg, a3gg; /* gamma/gamma second derivatives */
        private static double b1gg, b2gg, b3gg; /* gamma/gamma second derivatives */
        private static double c1gg, c2gg, c3gg; /* gamma/gamma second derivatives */
        private static double a1tp, a2tp, a3tp; /* theta/phi second derivatives */
        private static double b1tp, b2tp, b3tp; /* theta/phi second derivatives */
        private static double c1tp, c2tp, c3tp; /* theta/phi second derivatives */
        private static double a1tg, a2tg, a3tg; /* theta/gamma second derivatives */
        private static double b1tg, b2tg, b3tg; /* theta/gamma second derivatives */
        private static double c1tg, c2tg, c3tg; /* theta/gamma second derivatives */
        private static double a1pg, a2pg, a3pg; /* phi/gamma second derivatives */
        private static double b1pg, b2pg, b3pg; /* phi/gamma second derivatives */
        private static double c1pg, c2pg, c3pg; /* phi/gamma second derivatives */

        private static double cval; /* value of constraint equation */
        private static double[] cgrad = new double[3]; /* grad of constraint equation */

/* Returns value of penalty function at given angle. */
        public static double func3d(double[] coeffs /*[15]*/, /* five different 4-way products */
        ...
        )
        {
            ...
            sphi = Math.Sin(phi);
            cphi = Math.Cos(phi);
            sgamma = Math.Sin(gamma2);
            cgamma = Math.Cos(gamma2);
            onemcg = 1.0 - cgamma;
            ...
            var val = (a1 * a1 * a1 * a1 + a2 * a2 * a2 * a2 + a3 * a3 * a3 * a3) * coeffs[0];
...
/* Returns 1st derivative of penalty function at given angle. */
        public static void grad3d(double[] coeffs /*[15]*/, /* five different 4-way products */
        ...
        )
        {
            ...
            double Grad0()
            {
                var val = 4 * (a1t * a1 * a1 * a1 + a2t * a2 * a2 * a2 + a3t * a3 * a3 * a3) * coeffs[0];
            ...
            double Grad1()
            {
                var val = 4 * (a1p * a1 * a1 * a1 + a2p * a2 * a2 * a2 + a3p * a3 * a3 * a3) * coeffs[0];
            ...
            double Grad2()
            {
                var val = 4 * (a1g * a1 * a1 * a1 + a2g * a2 * a2 * a2 + a3g * a3 * a3 * a3) * coeffs[0];
...
/* Returns 2nd derivative of penalty function at given angle. */
        public static void hess3d(double[] coeffs /*[15]*/, /* five different 4-way products */
        ...
        double hess12()
        {
            ...
            hess[1][1] = hess11();
            hess[2][2] = hess22();
            hess[0][1] = hess[1][0] = hess01();
            hess[0][2] = hess[2][0] = hess02();
            hess[1][2] = hess[2][1] = hess12();
        }

        public static double constraint(double* coeffs2 /* coefficients for constraint eqn */
        )
        {
        ...
        public static void gradcon(double* coeffs2, /* coefficients for constraint eqn */
            double[] grad /*[3]*/ /* gradient returned */
        )
        {
            double gradcon0()
            {
                var val = (a1t * a2 * a3 + a1 * a2t * a3 + a1 * a2 * a3t) * coeffs2[0];
            ...
            double gradcon1()
            {
                var val = (a1p * a2 * a3 + a1 * a2p * a3 + a1 * a2 * a3p) * coeffs2[0];
            ...
            double gradcon2()
            {
                var val = (a1g * a2 * a3 + a1 * a2g * a3 + a1 * a2 * a3g) * coeffs2[0];
            ...
            cval += (a1 * b2 * c3 + a1 * c2 * b3 + b1 * a2 * c3 + b1 * c2 * a3 + c1 * a2 * b3 + c1 * b2 * a3) * coeffs2[9];
        ...
        public static void hesscon(double* coeffs2, /* coefficients for constraint eqn */
            double[][] hess /*[3][3]*/ /* hessian returned */
        )
        {
            double hesscon00()
