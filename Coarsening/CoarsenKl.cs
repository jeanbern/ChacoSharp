...
)
{
  ...
  if (DEBUG_COARSEN || DEBUG_TRACE) {
    Console.WriteLine("<Entering coarsen_kl, step={0:d}, nvtxs={1:d}, nedges={2:d}, vmax={3:d}>", step, nvtxs, nedges, vmax);
  }
  ...
  if (nvtxs <= vmax || give_up) {
    ...
    if (mkconnected) {
      ...
      if (DEBUG_CONNECTED) {
        Console.WriteLine("Enforcing connectivity on coarse graph");
  ...
  *pbndy_list = bndy_list;
  ...
  if (DEBUG_COARSEN) {
    Console.WriteLine(" Leaving coarsen_kl, step={0:d}", step);
  }
}
