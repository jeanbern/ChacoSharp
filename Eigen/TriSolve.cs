...
{
    public static unsafe class TriSolve
    {
   ...
   that all elements of the rhs except the first are 0. */
        public static void tri_solve(double* alpha, /* vector of Lanczos scalars */
        ...
        )
        {
            ...
            if (Math.Abs(d[1]) < tol)
            {
                throw new InvalidOperationException("ERROR: Zero pivot in tri_solve().");
            }
