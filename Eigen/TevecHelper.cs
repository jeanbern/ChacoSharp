...
using static ChacoSharp.Eigen.BiDir;
using static ChacoSharp.TinVit.TinvitHelper;

namespace ChacoSharp.Eigen
{
    public static unsafe class TevecHelper
    {
...
/* Finds eigenvector s of T and returns residual norm. */
        public static double Tevec(double* alpha, /* vector of Lanczos scalars */
        ...
        )
        {
            ...
            double[] w = new double[MAXDIMS + 1]; /* holds eigenvalue for tinvit */
            int[] index = new int[MAXDIMS + 1]; /* index vector for tinvit */
            int ierr; /* error flag for tinvit */
            int nevals; /* number of evals sought */
            int long_j; /* long copy of j for tinvit interface */
            ...
            if (j == 1)
            {
                residual = Math.Abs(alpha[1] - ritz);
            }
            ...
            if (residual > SRESTOL)
            {
                ...
                fixed (double* wPtr = &w[1])
                {
                    fixed (int* indexPtr = &index[1])
                    {
                        tinvit(&long_j, &long_j, &(alpha[1]), &(beta[1]), &(work[1]), &nevals, wPtr, indexPtr,
