...
{
    public static unsafe class SturmCount
    {
   ...
   first off-diagonal entry in T is beta[2]. We index such that it is beta[1]. */
        ...
        public static int sturmcnt(double* alpha, /* vector of Lanczos scalars */
        ...
        )
        {
            ...
            else
            {
                ...
                limit = Math.Sqrt(double.MaxValue);
                p[0] = 1;
                p[1] = alpha[1] - mu;
                pntr_p = &p[2];
                pntr_p1 = &p[1];
                ...
                for (i = j; i != 0; i--)
                {
                    if (*pntr_p != *pntr_p || Math.Abs(*pntr_p) > limit)
                    {
                        return (-1);
