...
)
{
  ...
  return (eguy);
        ...
        public static void update_mesh_edata(int                   vertex,     /* graph vertex being worked on */
...
)
{
  ...
  for (i = 0; i < 2; i++) { /* Have to adjust two edges. */
    ...
    if (eguy != null) {
      ...
      new_desire = (float)compute_mesh_edata(eguy, vdata, mesh_dims, comm_graph, node2vtx);
